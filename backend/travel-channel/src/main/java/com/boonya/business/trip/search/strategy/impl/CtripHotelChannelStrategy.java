...
@Slf4j
@Component
public class CtripHotelChannelStrategy extends AbstractChannelStrategy {

    @Override
    protected Order doPlaceOrder(Order order) {
        log.info("[携程] 酒店下单，订单号：{}", order.getOrderNo());

        // TODO: 调用携程酒店 API 下单

        log.info("[携程] 酒店下单成功，订单号：{}", order.getOrderNo());
        return order;
    }
