...
@Slf4j
@Component
public class TongchengHotelChannelStrategy extends AbstractChannelStrategy {

    @Override
    protected Order doPlaceOrder(Order order) {
        log.info("[同程] 酒店下单，订单号：{}", order.getOrderNo());

        // TODO: 调用同程 API 下单

        log.info("[同程] 酒店下单成功，订单号：{}", order.getOrderNo());
        return order;
    }
